...
const Rent = () => {
  ...
  if (!rentedRoom) {
    return (
      <div className="text-center text-lg text-gray-600">Its Empty here</div>
    );
  }
        ...
        <div className="mt-4 space-y-4">
          {rentedRoom.map((rentEntry) =>
            rentEntry.status === "Unpaid" ? (
              ...
              >
                <p>
                  <strong>Rent Amount:</strong> {rentEntry.amount}
                </p>
                <p>
                  <strong>For Month:</strong>{" "}
                  {new Date(rentEntry.forMonth).toLocaleDateString()}
                </p>
                <p>
                  <strong>Status:</strong> {rentEntry.status}
                </p>
                <button
